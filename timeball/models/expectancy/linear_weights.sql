...
WITH agg AS (
    SELECT DISTINCT ON (trans.season, trans.league, pa.plate_appearance_result)
        trans.season,
        trans.league,
    ...
    WHERE event_key NOT IN (SELECT event_key FROM {{ ref('stg_event_baserunning_plays') }})
        ...
        AND trans.season >= 1914
    WINDOW 
        all_league AS (PARTITION BY trans.season, trans.league),
        result AS (PARTITION BY trans.season, trans.league, pa.plate_appearance_result)
),
