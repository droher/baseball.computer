...
final AS (
    SELECT
        ...
        COALESCE(ct.broad_classification = 'GroundBall' AND hl.general_location IS NOT NULL,
            FALSE) AS has_general_location_groundball,
        COALESCE(ct.broad_classification = 'AirBall' AND hl.general_location IS NOT NULL,
            FALSE) AS has_general_location_airball,
        COALESCE(ct.broad_classification = 'GroundBall' AND lt.is_mid_position,
            FALSE) AS has_mid_position_groundball,
        COALESCE(ct.broad_classification = 'AirBall' AND lt.is_mid_position,
            FALSE) AS has_mid_position_airball,
        COALESCE(ct.broad_classification = 'GroundBall' AND hl.depth != 'Default',
            FALSE) AS has_depth_groundball,
        COALESCE(ct.broad_classification = 'AirBall' AND hl.depth != 'Default',
            FALSE) AS has_depth_airball,
        COALESCE(ct.broad_classification = 'GroundBall' AND hl.angle != 'Default',
            FALSE) AS has_angle_groundball,
        COALESCE(ct.broad_classification = 'AirBall' AND hl.angle != 'Default',
            FALSE) AS has_angle_airball,
        COALESCE(ct.broad_classification = 'GroundBall' AND hl.strength != 'Default',
            FALSE) AS has_strength_groundball,
        COALESCE(ct.broad_classification = 'AirBall' AND hl.strength != 'Default',
            FALSE) AS has_strength_airball,
