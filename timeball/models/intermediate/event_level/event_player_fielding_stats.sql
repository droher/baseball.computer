...
WITH final AS (
    SELECT
        ...
        CASE WHEN e.batted_to_fielder = personnel.fielding_position THEN 1 ELSE 0 END::UTINYINT AS balls_hit_to,
        ...
        CASE WHEN personnel.fielding_position IN (1, 2) THEN e.stolen_bases ELSE 0 END::UTINYINT AS stolen_bases,
        CASE WHEN personnel.fielding_position IN (1, 2) THEN e.caught_stealing ELSE 0 END::UTINYINT AS caught_stealing,
        CASE WHEN personnel.fielding_position IN (1, 2) AND fp.assists > 0 THEN e.pickoffs ELSE 0 END::UTINYINT AS pickoffs,
