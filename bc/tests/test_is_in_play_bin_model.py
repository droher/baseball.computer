"""Unit tests for the binary is-in-play model factory + Scorer contract."""
...
from python_models.ml.model_is_in_play_bin import build_model as build_in_play_model
...
def test_binary_build_model_emits_single_sigmoid_output() -> None:
    ...
    assert model.name == IS_IN_PLAY_BIN.name
...
def _make_binary_scorer(num_rows_vocab: int = 50) -> Scorer:
    means, variances = _numeric_stats()
    model = build_model(
        target_spec=IS_IN_PLAY_BIN,
        vocab_sizes=_vocab_sizes(),
    ...
    for col in HIGH_CARD_CATEGORICAL:
        vocabularies[col] = Vocabulary(
            column=col, values=tuple(f"id_{i}" for i in range(num_rows_vocab))
        )
    for col in LOW_CARD_CATEGORICAL:
        vocabularies[col] = Vocabulary(
