...
_AUDITS = [
        ...
        "not_null",
        {
            "columns": exp.Tuple(
                expressions=[
                    ...
                    exp.column("predicted_class"),
                    exp.column("predicted_class_proba"),
    ...
    (
        "bounded_range",
        {
            "column": exp.column("predicted_class_proba"),
...
@model(
    "main_models.predictions_plate_appearance_cat",
    kind="FULL",
    columns={
        "event_key": "UINTEGER",
        "predicted_class": "VARCHAR",
        "predicted_class_proba": "DOUBLE",
...
def execute(
    ...
    scorer = load_scorer("plate_appearance_cat")
    # Streaming via a sibling DuckDB connection on the same database.
    # SQLMesh's adapter exposes a `DuckDBPyConnection`; calling
    # `.cursor()` on a DuckDBPyConnection returns ANOTHER
    # DuckDBPyConnection that shares the underlying database but holds
    # its own scan state. Per dbt-duckdb PR #269, this gives the read
    # path its own handle so it survives concurrent INSERTs from the
    # writer. The variable is still called a "connection" because
    # that's what DuckDB returns; "cursor" is the method name only.
