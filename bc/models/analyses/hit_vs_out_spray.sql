MODEL (
  name main_models.hit_vs_out_spray,
  kind VIEW
);
...
INNER JOIN main_models.calc_batted_ball_type AS c USING (game_id, event_key)
INNER JOIN main_models.event_states_full p USING (game_id, event_key)
