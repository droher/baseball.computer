MODEL (
  name main_models.fielder_advance_expectancy,
  kind VIEW
);
...
WITH base_data AS (
    SELECT
        ...
        END AS is_out,
    FROM main_models.event_baserunning_stats AS b
    INNER JOIN main_models.stg_events AS e USING (event_key)
    INNER JOIN main_seeds.seed_plate_appearance_result_types AS part USING (plate_appearance_result)
    INNER JOIN main_models.game_start_info AS g ON g.game_id = e.game_id
    LEFT JOIN main_models.calc_batted_ball_type AS bbt USING (event_key)
    LEFT JOIN main_models.event_player_fielding_stats AS f
