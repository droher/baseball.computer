MODEL (
  name main_models.hit_vs_out_spray_air,
  kind VIEW
);
...
FROM main_models.game_data_completeness AS g
INNER JOIN main_models.calc_batted_ball_type AS c USING (game_id)
INNER JOIN main_models.event_offense_stats AS e USING (event_key)
INNER JOIN main_models.event_states_full p USING (event_key)
