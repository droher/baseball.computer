MODEL (
  name main_models.event_completeness_batted_balls,
  kind VIEW
);
...
WITH final AS (
    ...
    LEFT JOIN main_seeds.seed_hit_location_categories AS lt USING (batted_location_general)
    LEFT JOIN main_seeds.seed_plate_appearance_trajectories AS ct USING (batted_trajectory)
)
