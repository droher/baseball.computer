MODEL (
  name main_models.babip_by_count,
  kind VIEW
);
...
WITH players AS (
    ...
    SUM(hits) as h,
    h / ab AS avg
FROM main_models.event_states_full s
JOIN main_models.event_offense_stats o USING (event_key)
JOIN main_models.calc_batted_ball_type b USING (event_key)
