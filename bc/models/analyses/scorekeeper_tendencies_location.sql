MODEL (
  name main_models.scorekeeper_tendencies_location,
  kind VIEW
);
...
WITH final AS (
    ...
    FROM main_models.event_offense_stats s
    LEFT JOIN main_models.game_scorekeeping k USING (game_id)
    INNER JOIN main_models.event_states_full f USING (event_key)
    LEFT JOIN main_models.stg_events AS e USING (event_key)
    LEFT JOIN main_models.leverage_index li USING (win_expectancy_start_key)
