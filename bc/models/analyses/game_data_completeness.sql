MODEL (
  name main_models.game_data_completeness,
  kind VIEW
);
...
WITH batted_balls AS (
    ...
    FROM main_models.calc_batted_ball_type AS bb
    LEFT JOIN main_seeds.seed_plate_appearance_result_types AS rt USING (plate_appearance_result)
    GROUP BY 1
),
...
fielding_credit AS (
    ...
    LEFT JOIN main_models.event_completeness_fielding_credit AS fc USING (event_key)
    GROUP BY 1
),
