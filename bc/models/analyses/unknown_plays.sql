MODEL (
  name main_models.unknown_plays,
  kind VIEW
);
...
team_season_overall_coverage AS (
    ...
    FROM t 
    INNER JOIN main_models.stg_games g USING (game_id)
    INNER JOIN main_models.team_game_start_info gt USING (game_id, team_id)
