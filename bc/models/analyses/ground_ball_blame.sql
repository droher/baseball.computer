MODEL (
  name main_models.ground_ball_blame,
  kind VIEW
);
...
WITH base AS (
    ...
    INNER JOIN main_models.calc_batted_ball_type AS c USING (event_key)
    INNER JOIN main_models.event_states_full AS s USING (event_key)
