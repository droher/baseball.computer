MODEL (
  name main_models.fielder_location_shares,
  kind VIEW
);
...
WITH t AS (
    ...
    FROM main_models.event_offense_stats AS e
    INNER JOIN main_models.calc_batted_ball_type AS c USING (event_key, game_id)
    INNER JOIN main_models.event_states_full p USING (event_key, game_id)
