{# Hardcoded column lists for the Phase 1 SQLMesh metric models.

   Why hardcoded:
     The original `metrics_table_generator.sql` introspected each upstream
     `player_*_game_*_stats` model with `adapter.get_columns_in_relation()`
     to derive its int-column list. SQLMesh's data-type parser cannot
     handle DuckDB ENUM literals, so all 9 metric models failed to render
     under the dbt-import path
     (see notes/spikes/01_sqlmesh_init/RESULT.md).

   Source of truth:
     A fresh dbt build of the relevant game-level model, filtered by the
     same predicate as `metrics_table_generator.sql:35-39` (INT cols, drop
     *_id / *_key / *_position / surplus_box* / season).

   If the upstream player_*_game_* model adds a new int counter, append
   it here and re-run the diff harness — otherwise it gets silently
   dropped from the metric output.
#}

{% macro offense_int_cols() %}
    {{ return([
...
{# team_game_start_info columns NOT in any of the player_*_game_*_stats
   upstream models. Same list across all 3 kinds because the only shared
   columns are the join keys (team_id, game_id), handled by USING. #}
{% macro metric_game_cols() %}
    {{ return([
